...
d3.tsv("data/film.tsv")
	.row(function (d, i) 
	{
		...
		samedirector[i]=d.Director;    //Load directors,actors,actress to 3 arrays
                sameactor[i]=d.Actor;
		sameactress[i]=d.Actress;
...
function getSameDirector()
{
	var rowNumsDir = [];
        ...
        if((samedirector[i]===dataset[currMovie].director) && (i!=currMovie) && (samedirector[i]!=""))
           {rowNumsDir.push(i);}  
       }
        console.log(rowNumsDir);
	return rowNumsDir;
}
...
function getSameActor()
{
	var rowNumsAct = [];	
	for(var i=1;i<dataset.length;++i){ 
        if((sameactor[i]===dataset[currMovie].actor) && (i!=currMovie) && (sameactor[i]!=""))    
           {rowNumsAct.push(i);}  
       }
        console.log(rowNumsAct);
	return rowNumsAct;
}
...
function getSameActress()
{
	var rowNumsActr = [];	
	for(var i=1;i<dataset.length;++i){    
        if((sameactress[i]===dataset[currMovie].actress) && (i!=currMovie) && (sameactress[i]!="")){
           rowNumsActr.push(i);}  
       }
        console.log(rowNumsActr);
        console.log(sameactress);
